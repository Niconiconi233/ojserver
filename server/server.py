...
from utils import server_info, logger, token, ProblemIOMode, init_language_config, init_spj_config, init_spj_compile
...
class JudgeServer:
    ...
    def language(cls):
        spj_languages = [];
        spj_languages.append(languages[0])
        spj_languages.append(languages[1])
        data = {"languages": languages, "spj_languages":spj_languages}
        return data

    @classmethod
    def judge(cls, language_type, src, max_cpu_time, max_memory, test_case_id=None, test_case=None,
              spj_version=None, spj_config=None, spj_src=None, output=False, io_mode=None):
        if not io_mode:
        ...
        # init
        language_config = init_language_config(language_type)
        if not language_config:
            raise JudgeClientError("unkown language type")
        ...
        if is_spj:
            ...
            if not os.path.isfile(spj_exe_path):
                ...
                spj_compile_config = init_spj_compile
    ...
    @classmethod
    def compile_spj(cls, spj_version, src):
        spj_compile_config = init_spj_compile()
...
def server(path):
    if path in ("judge", "language", "ping", "compile_spj"):
